...
type CommentRow = SideBetComment & {
  profiles?: { display_name: string | null; email: string; public_slug: string | null; avatar_url: string | null } | null;
};
...
export default function SideBetsPage() {
  ...
  const [hiddenComments, setHiddenComments] = useState<Record<number, boolean>>({});
  ...
  const load = useCallback(async () => {
    ...
    const [teamsRes, betsRes, commentsRes] = await Promise.all([
      ...
      supabase
        ...
        .select("id,bet_id,user_id,message,created_at")
    ...
    const commentProfilesRes =
      commentUserIds.length > 0
        ? await supabase.from("profiles").select("id,display_name,email,public_slug,avatar_url").in("id", commentUserIds)
        : { data: [], error: null };

    const commentProfileMap = new Map(
      (commentProfilesRes.data ?? []).map((commentProfile) => [commentProfile.id, commentProfile])
    );
  ...
  async function submitComment(betId: number) {
    ...
    setHiddenComments((prev) => ({ ...prev, [betId]: false }));
    setOpenCommentComposers((prev) => ({ ...prev, [betId]: false }));
    await load();
    setCommentSubmitting(null);
  }
          ...
          <div className="rounded-lg border border-white/10 bg-white/5 p-3">
            {!commentsHidden ? (
              <div className="space-y-2">
                {list.map((c) => {
                  const commentName = c.profiles?.display_name || c.profiles?.email || "Player";
                  return (
                    <div key={c.id} className="ml-3 rounded-lg border border-white/10 bg-slate-950/50 p-3">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <a
                              ...
                              href={`/players/${c.profiles?.public_slug || c.user_id}`}
                            >
                              {commentName}
                            </a>
                          </div>
            ...
            {composerOpen ? (
              <div className="mt-3 flex flex-col gap-2">
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  >
                    {commentSubmitting === betId ? "Posting..." : "Post Reply"}
                  </button>
                </div>
              </div>
            ) : null}
